  ...
  return (
    <div className="flex h-screen bg-[#F5F5F0] text-[#1a1a1a] font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-black/5 px-2 py-2 flex justify-around items-stretch md:hidden z-40 pb-safe">
          {filteredMenu.slice(0, 5).map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={cn(
                "flex-1 min-w-0 flex flex-col justify-center items-center px-1 py-2 rounded-xl transition-all min-h-[64px]",

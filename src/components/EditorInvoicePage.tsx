...
function StatusBadge({ status }: { status: EditorInvoice['status'] }) {
  ...
  if (status === 'approved') {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[13px] font-bold border border-blue-200">
  ...
  if (status === 'payment_processing') {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-50 text-purple-700 text-[13px] font-bold border border-purple-200">
...
export default function EditorInvoicePage({ userProfile }: { userProfile: UserProfile | null }) {
  ...
  if (!EDITOR_INVOICING_ENABLED) {
    return (
      <div className="readability-surface bg-white rounded-3xl shadow-sm border border-black/5 p-12 text-center space-y-3">
        <Receipt size={30} className="mx-auto text-[#5A5A40]" />
        <h3 className="text-lg font-bold serif text-[#5A5A40]">請款功能準備中</h3>
        <p className="text-sm text-gray-500">目前仍沿用原本對帳方式；資料盤點完成後會由公司通知正式啟用。</p>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="readability-surface space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="bg-white px-4 py-2 rounded-2xl border border-black/5 shadow-sm">
            <p className="text-[13px] font-bold text-gray-500">本月可請款</p>
            <p className="text-lg font-bold leading-none text-[#5A5A40]">
              {billable.length} <span className="text-sm font-normal text-gray-500">支</span>
            </p>
          </div>
          <div className="bg-white px-4 py-2 rounded-2xl border border-[#5A5A40]/20 shadow-sm">
            <p className="text-[13px] font-bold text-gray-500">金額合計</p>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[13px] font-bold text-gray-500 w-10 shrink-0">月份</span>
        {monthOptions.map(m => (
          <button
            key={m}
          ...
          {billable.length > 0 && (
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="px-3 py-1 rounded-full text-[13px] font-bold bg-white border border-black/5 text-gray-500 hover:text-[#5A5A40]"
              >
                全選
              </button>
              <button
            ...
            {groups.map(g => (
              <div key={g.vendorId}>
                <div className="px-5 py-2 bg-black/[0.015] flex items-center justify-between">
                  <span className="text-[13px] font-bold text-[#5A5A40]">{g.vendorName}</span>
                ...
                {g.assets.map(a => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-base text-gray-600 truncate">{a.title}</p>
                      <p className="text-[13px] text-gray-500 mt-0.5">
        ...
        {billable.length > 0 && (
          <div className="px-5 py-3 border-t border-black/5 bg-white flex items-center justify-between gap-3 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-[#5A5A40] text-white px-5 py-2 rounded-xl text-sm font-bold shadow-sm hover:bg-[#4a4a35] transition-all disabled:opacity-40"
            >
              <Send size={14} />
              送出請款
            </button>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-black/5 overflow-hidden">
        <div className="px-5 pt-4 pb-3 border-b border-black/5">
          ...
          <p className="text-[13px] text-gray-500 mt-0.5">送出後金額不能再改，如果有錯請聯繫我們。</p>
        </div>
            ...
            {monthInvoices.map(inv => (
              <div key={inv.id} className="px-5 py-3 space-y-2">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-base font-bold text-[#5A5A40] flex items-center gap-1.5">
                      <Wallet size={14} /> {money(inv.totalAmount)}
                    </span>
                    <span className="text-[13px] text-gray-500">{inv.itemCount} 支</span>
          ...
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden">
                ...
                {chosen.map(a => (
                  <div key={a.id} className="py-2 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-[13px] font-bold text-[#5A5A40]">{vendorName(a.vendorId)}</p>
                      <p className="text-sm text-gray-600 truncate">{a.title}</p>
                      <p className="text-[13px] text-gray-500">
                        {a.cloudUploadedAt ? `${format(parseISO(a.cloudUploadedAt), 'MM/dd')} 上傳雲端` : ''}
                      </p>
                    </div>
                    <span className="text-base font-bold text-[#5A5A40] shrink-0">{money(feeOf(a))}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-black/5 bg-black/[0.015] space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">{monthLabel(month)}請款合計（{chosen.length} 支）</span>
              ...
              <div className="flex items-center justify-end gap-2">
                <button
                  ...
                  disabled={submitting}
                  className="px-4 py-2 rounded-xl text-sm font-bold bg-white border border-black/10 text-gray-500 hover:text-[#5A5A40] disabled:opacity-40"
                >
                  再檢查一次
                </button>
                <button
          ...
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm overflow-hidden">
            ...
            <div className="px-6 pb-5">
              <button
                ...
                className="w-full bg-[#5A5A40] text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-[#4a4a35] transition-all"
              >
                知道了
              </button>
            </div>
